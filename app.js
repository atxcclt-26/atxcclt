...
// OJO: COMERCIO y DETALLE estaban cruzados en el fichero; las etiquetas van intercambiadas a propósito.
const BASE_HEADER = [
  "BANCO", "FECHA CARGO", "FECHA GASTO", "DETALLE", "COMERCIO",
  "SINO", "€", "CATEGORIA", "COMENTARIO", "MONEDA", "VALOR MONEDA", "VALOR ORIGINAL",
  "FECHA", "FICHERO"
];

// Columnas
const HIDDEN_COLS = ["VALOR MONEDA", "VALOR ORIGINAL", "FECHA", "FICHERO"];   // no se muestran ni se filtran (VALOR ORIGINAL se integra en €)
const COL_SINO = "SINO", COL_CAT = "CATEGORIA", COL_COM = "COMENTARIO";  // columnas de origen que disparan botones/color
const COL_EUR = "€", COL_VALOR = "VALOR ORIGINAL", COL_MON = "MONEDA", COL_MERC = "COMERCIO";
...
// Parsea números con coma o punto decimal (y separador de miles); null si no es numérico.
function parseNum(v) {
  let s = (v == null ? "" : String(v)).replace(/\s|€/g, "");
  if (s === "") return null;
...
// Formatea con 2 decimales (coma decimal); si no es numérico devuelve el valor tal cual.
function fmt2(v) { const n = parseNum(v); return n == null ? (v || "") : n.toFixed(2).replace(".", ","); }
...
function prepararColumnas() {
  ...
  G.idx = {
    ...
    EUR: col(COL_EUR), VALOR: col(COL_VALOR), MON: col(COL_MON), MERC: col(COL_MERC),
...
function celda(row, c, idx) {
  ...
  if (c === G.idx.MERC) {
    // Fondo rojo si el COMERCIO contiene algún símbolo de interrogación.
    const cls = /[?¿]/.test(row[c] || "") ? ' class="cell-red"' : "";
